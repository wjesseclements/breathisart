          ...
          {customPatterns.map((pattern) => (
            <li key={pattern.id} className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <p className="truncate text-sm text-slate-200">{pattern.name}</p>
                <p className="truncate text-xs text-slate-500">{pattern.tagline}</p>
              </div>
              <button
                type="button"
                onClick={() => onEdit(pattern)}
                className="shrink-0 rounded-full border border-night-mist px-4 py-1 text-xs text-slate-400 transition-colors hover:border-breath-teal hover:text-slate-200 focus-visible:outline focus-visible:outline-2 focus-visible:outline-breath-teal"
              >
                Edit
              </button>
            </li>
